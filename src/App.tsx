...
export default function App(){
  ...
  const bindAudio=useCallback((element:HTMLAudioElement|null)=>audio.attach(element),[audio]);
  ...
  const act=(action:Action)=>{apply(action);};
  const repeat=()=>{sayState(stateRef.current);};
  const toggleMic=()=>{
  ...
  return <div className={`app ${emergency?'is-emergency':''}`}>
    ...
    <main className="workspace">
      ...
      <section className="checklist-audio" aria-label="Checklist audio"><div><label htmlFor="checklist-audio">Checklist audio</label><p>Tap Read item or Repeat to load the current recording.</p></div><audio id="checklist-audio" aria-label="Checklist audio player" ref={bindAudio} controls playsInline preload="auto"/></section>
